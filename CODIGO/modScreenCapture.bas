...
'Obtenido de Argentum Online Oficial
'WWW.ARGENTUMONLINE.COM.AR
'Reprogramado y adaptado por CHOTS para LapsusAO 2.0
...
On Error GoTo Err:
    ...
    frmScreenshots.Picture1.Width = 12050
    frmScreenshots.Picture1.Height = 9050
    
    BitBlt frmScreenshots.Picture1.hdc, 0, 0, 800, 600, hdcc, 0, 0, SRCCOPY
    
    Call ReleaseDC(frmMain.hWnd, hdcc)
    
    hdcc = INVALID_HANDLE
        
    file = App.Path & "\Screenshoots\LapsusAO " & Format(Now, "DD-MM-YYYY hh-mm-ss") & ".jpg"
...
Err:
    ...
    If hdcc <> INVALID_HANDLE Then _
        Call ReleaseDC(frmMain.hWnd, hdcc)
End Sub
Public Function FullScreenCapture() As String
    Dim hWnd As Long
    Dim file As String
    Dim sI As String
    Dim c As New cDIBSection
    ...
    BitBlt frmScreenshots.Picture1.hdc, 0, 0, Screen.Width \ Screen.TwipsPerPixelX, Screen.Height \ Screen.TwipsPerPixelY, GetDC(GetDesktopWindow), 0, 0, SRCCOPY
    
    Call ReleaseDC(frmMain.hWnd, GetDC(GetDesktopWindow))
